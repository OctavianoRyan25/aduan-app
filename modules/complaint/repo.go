...
func (r *complaintRepository) GetAllComplaint() ([]Complaint, error) {
	var complaints []Complaint
	if err := r.db.Preload("Images").Find(&complaints).Error; err != nil {
		return nil, err
	}
...
func (r *complaintRepository) GetComplaintByID(id int) (*Complaint, error) {
	...
	if err := r.db.Preload("Images").Where("id = ?", id).First(&complaint).Error; err != nil {
		return nil, err
	}
